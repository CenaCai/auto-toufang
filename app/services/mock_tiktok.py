"""Mock TikTok Ads client for development/testing."""
...
class MockTikTokClient(AdsClient):
    """Simulates TikTok Ads API responses.

    TikTok mock tends to have lower CPC but more variable conversion rates,
    reflecting typical short-video ad platform patterns.
    """
    ...
    async def get_campaign_stats(
        ...
        # 25% chance of bad period (TikTok can be volatile)
        is_bad_period = random.random() < 0.25

        spend = max(0, random.gauss(hourly_budget, hourly_budget * 0.35))
        impressions = int(max(0, random.gauss(spend * 400, spend * 100)))  # high impressions, cheap CPM
        clicks = int(max(0, impressions * random.gauss(0.015, 0.005)))  # lower CTR

        if is_bad_period:
            installs = int(max(1, clicks * random.gauss(0.02, 0.008)))
            revenue = installs * random.gauss(1.2, 0.4)
        else:
            installs = int(max(1, clicks * random.gauss(0.10, 0.03)))
            revenue = installs * random.gauss(3.8, 1.0)
